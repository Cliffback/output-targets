...
Event names that contain `-` or `/` are exposed as valid wrapper property names with Angular output
metadata aliases, so a DOM event named `my-event` can still be bound as `(my-event)="handler($event)"`.
